...
- **DO THIS FIRST tomorrow**: draw a bright HUD dot at `x0 + fc.x()*scale, y0 + fc.y()*scale` and see if it lands on the cyan pillar. If NOT → `guiGraphics.blit(TEXTURE, x0,y0, s,s, 0f,1f,0f,1f)` does NOT map texture pixel (tx,ty) to screen `(x0 + tx*s/SIZE, y0 + ty*s/SIZE)` as assumed. Then inspect that exact `blit` overload's semantics in THESE mappings (the 4 trailing floats may be `(u0,v0,u1,v1)` or region-px, not `(minU,maxU,minV,maxV)`; the dest rect or texture-dim assumption may differ). Cross-check vs the 2D map's blit in `AbyssWorldMapScreen` (full-screen). Also confirm `s=Math.min(width,height)` is in the same coord space the blit uses. Fixing the blit-mapping assumption should make the HUD land on the cyan. Then strip cyan + `projectOffset`, and refresh the beta.
- Key files: `OrbitScene.java` (`projectHud` + hud-camera snapshot + cyan debug), `OrbitView.java` (`drawArm`/`drawLabel`/`focusOccluded`/marker all via `projectHud`; marker centre `mcx=x0+fc.x()*scale`).
